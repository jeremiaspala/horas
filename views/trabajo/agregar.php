...
function agregar(id_persona){
        $.ajax({
		type: "post",
		url: "<?=base_url?>trabajo/sumar&trabajo_id=<?=intval($_REQUEST['id'])?>&persona_id="+id_persona,
		success: function(data){
		if(data==="no"){
                    alert("No se ha podido cargar el host!");
		}else{
                    obtenerEn('empleados', '<?=base_url?>trabajo/devolver&id=<?=intval($_REQUEST['id'])?>')
                }
	}
	});
}
...
function eliminar(id){
    ...
    if(a === true){
        $.ajax({
		type: "post",
		url: "<?=base_url?>trabajo/restar&id="+id,
		success: function(data){
		if(data==="ok"){
